...
export default function DeleteAccount() {
  ...
  return (
    <div style={{ minHeight: "100vh", padding: "60px 16px", maxWidth: 800, margin: "0 auto", backgroundColor: "var(--background)" }}>
      {/* ── HEADER ── */}
      ...
      {step === 1 && (
        <div style={{ backgroundColor: "var(--card)", border: "1px solid var(--border)", borderRadius: 20, padding: "32px", boxShadow: "0 4px 6px -1px rgba(0,0,0,0.05)" }}>
          ...
          <h3 style={{ fontSize: 18, fontWeight: 700, color: "var(--foreground)", marginBottom: 16 }}>What happens when you delete your account?</h3>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 32 }}>
            {[
              ...
              <div key={item.title} style={{ display: "flex", gap: 12 }}>
                <div style={{ width: 32, height: 32, borderRadius: 8, backgroundColor: "var(--secondary)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <item.icon style={{ width: 16, height: 16, color: "var(--muted-foreground)" }} />
                </div>
                <div>
                  <p style={{ fontWeight: 600, fontSize: 14, color: "var(--foreground)", margin: "0 0 2px" }}>{item.title}</p>
          ...
          <button 
            onClick={() => setStep(2)}
            style={{ width: "100%", background: "#dc2626", color: "var(--card)", fontWeight: 700, padding: "14px", borderRadius: 12, border: "none", cursor: "pointer", fontSize: 15, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}
          >
      ...
      {step === 2 && (
        <div style={{ backgroundColor: "var(--card)", border: "1px solid var(--border)", borderRadius: 20, padding: "32px" }}>
          <h3 style={{ fontSize: 20, fontWeight: 700, color: "var(--foreground)", marginBottom: 8 }}>Final Confirmation</h3>
          <p style={{ fontSize: 14, color: "var(--muted-foreground)", marginBottom: 24 }}>Please provide your registered email/phone to initiate the deletion process.</p>
          
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            <div>
              <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "var(--foreground)", marginBottom: 6 }}>Email Address or Phone Number *</label>
              <input 
                required
                type="text" 
                ...
                style={{ width: "100%", padding: "12px", borderRadius: 8, border: "1px solid var(--border)", fontSize: 14, boxSizing: "border-box" }}
              />
            </div>
            <div>
              <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "var(--foreground)", marginBottom: 6 }}>Why are you leaving? (Optional)</label>
              <textarea 
                placeholder="We would love to hear your feedback..."
                value={reason}
                onChange={(e) => setReason(e.target.value)}
                style={{ width: "100%", padding: "12px", borderRadius: 8, border: "1px solid var(--border)", fontSize: 14, height: "100px", resize: "none", boxSizing: "border-box" }}
              />
            </div>
            ...
            <div style={{ display: "flex", gap: 12 }}>
              <button 
                type="button"
                onClick={() => setStep(1)}
                style={{ flex: 1, backgroundColor: "var(--secondary)", color: "var(--foreground)", fontWeight: 600, padding: "12px", borderRadius: 10, border: "none", cursor: "pointer" }}
              >
                Go Back
              </button>
              <button 
                type="submit"
                style={{ flex: 2, background: "#dc2626", color: "var(--card)", fontWeight: 700, padding: "12px", borderRadius: 10, border: "none", cursor: "pointer" }}
              >
      ...
      {step === 3 && (
        <div style={{ backgroundColor: "var(--card)", border: "1px solid var(--border)", borderRadius: 20, padding: "48px 32px", textAlign: "center" }}>
          ...
          <h3 style={{ fontSize: 24, fontWeight: 700, color: "var(--foreground)", marginBottom: 12 }}>Request Submitted</h3>
          <p style={{ fontSize: 15, color: "var(--muted-foreground)", lineHeight: 1.6, marginBottom: 32 }}>
          ...
          <a href="/" style={{ background: "#2563eb", color: "var(--card)", fontWeight: 600, padding: "12px 32px", borderRadius: 10, textDecoration: "none", fontSize: 14 }}>
            Return to Homepage
          </a>
        </div>
      )}
      ...
      {/* ── FOOTER INFO ── */}
      <div style={{ marginTop: 48, padding: "24px", backgroundColor: "var(--background)", borderRadius: 16, border: "1px solid var(--border)" }}>
        <h4 style={{ fontSize: 15, fontWeight: 700, color: "var(--foreground)", marginBottom: 8, display: "flex", alignItems: "center", gap: 8 }}>
