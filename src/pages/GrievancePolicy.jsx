  ...
  return (
    <div style={{ display: "flex", gap: 16 }}>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0 }}>
        <div style={{ width: 40, height: 40, borderRadius: "50%", background: color, color: "var(--card)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 14 }}>
          {num}
        </div>
        {!isLast && <div style={{ width: 2, height: 48, background: "var(--border)", marginTop: 4 }} />}
      </div>
      <div style={{ paddingBottom: isLast ? 0 : 0, paddingTop: 8 }}>
        <p style={{ fontWeight: 600, fontSize: 14, color: "var(--foreground)", margin: "0 0 4px" }}>{title}</p>
        <p style={{ fontSize: 13, color: "var(--muted-foreground)", margin: "0 0 20px" }}>{desc}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ minHeight: "100vh", padding: "48px 16px", maxWidth: 896, margin: "0 auto" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 40 }}>
        ...
        <p style={{ color: "var(--muted-foreground)", fontSize: 14, maxWidth: 540, margin: "0 auto" }}>
      ...
      <div style={{ backgroundColor: "var(--card)", border: "1px solid var(--border)", borderRadius: 16, padding: 24, marginBottom: 20 }}>
        ...
        <div style={{ backgroundColor: "var(--background)", borderRadius: 12, padding: 20, border: "1px solid var(--border)" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            {[
              ...
              <div key={item.label}>
                ...
                <p style={{ margin: "4px 0 0", fontWeight: 600, fontSize: 14, color: "var(--foreground)" }}>
                  {item.isLink
      ...
      <div style={{ backgroundColor: "var(--card)", border: "1px solid var(--border)", borderRadius: 16, padding: 24, marginBottom: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          {grievanceTypes.map((item) => (
            <div key={item.cat} style={{ display: "flex", alignItems: "flex-start", gap: 10, backgroundColor: "var(--background)", borderRadius: 10, border: "1px solid var(--border)", padding: 12 }}>
              <CheckCircle style={{ width: 16, height: 16, color: "#059669", flexShrink: 0, marginTop: 2 }} />
              <div>
                <p style={{ fontWeight: 600, fontSize: 13, color: "var(--foreground)", margin: "0 0 2px" }}>{item.cat}</p>
      ...
      {/* ── PROCESS ── */}
      <div style={{ backgroundColor: "var(--card)", border: "1px solid var(--border)", borderRadius: 16, padding: 24, marginBottom: 20 }}>
        <h2 style={{ fontSize: 20, fontWeight: 700, color: "var(--foreground)", marginBottom: 20, display: "flex", alignItems: "center", gap: 8 }}>
      ...
      {/* ── ESCALATION ── */}
      <div style={{ backgroundColor: "var(--card)", border: "1px solid var(--border)", borderRadius: 16, padding: 24, marginBottom: 20 }}>
        <h2 style={{ fontSize: 20, fontWeight: 700, color: "var(--foreground)", marginBottom: 8 }}>Escalation Path</h2>
        <p style={{ fontSize: 14, color: "var(--muted-foreground)", marginBottom: 16 }}>
          If unsatisfied with our resolution within 30 days, you may escalate to:
        </p>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {escalations.map((e) => (
            <div key={e.name} style={{ border: "1px solid var(--border)", borderRadius: 10, padding: "14px 16px" }}>
              <p style={{ fontWeight: 600, fontSize: 14, color: "var(--foreground)", margin: "0 0 4px" }}>{e.name}</p>
      ...
      <div style={{ backgroundColor: "var(--card)", border: "1px solid var(--border)", borderRadius: 16, padding: 24, marginBottom: 32 }}>
        ...
        <div style={{ backgroundColor: "var(--background)", borderRadius: 10, padding: 16, border: "1px solid var(--border)", fontFamily: "monospace", fontSize: 12, color: "var(--foreground)", lineHeight: 1.8 }}>
      ...
      <div style={{ background: "linear-gradient(135deg, #dc2626, #f43f5e)", borderRadius: 16, padding: "32px 24px", textAlign: "center" }}>
        <h2 style={{ fontSize: 20, fontWeight: 700, color: "var(--card)", margin: "0 0 8px" }}>File a Formal Grievance</h2>
        ...
        <a href="mailto:grievance@doorspitals.com" style={{ display: "inline-block", backgroundColor: "var(--card)", color: "#dc2626", fontWeight: 600, padding: "10px 24px", borderRadius: 999, fontSize: 14, textDecoration: "none" }}>
